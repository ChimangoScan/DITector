#!/usr/bin/env python3
"""exposure_bootstrap.py — popula a queue do scanners com top-N targets por exposure.

Mudanças vs seed atual do scanners:
  * image identifier vira "<ns>/<name>:<tag>@<digest>"  (digest-pinned)
  * fila é capada em --top-n (default 500k) por exposure desc
  * preserva done/running/skipped/failed/reports já existentes
  * cria tabela exposure_state com o watermark inicial

Idempotente: INSERT OR IGNORE em (image). Run again só adiciona o que faltar.

Pré-requisitos no host onde roda (<your-host>):
  - jsonl ranqueado: scanners/data/ditector_exposure_ranked.jsonl
    (saída de scripts/compute_exposure_ranking.py)
...
Sem --dry-run executa de verdade. Sempre faz baseline+after diff e aborta com
exit≠0 se done/running diminuírem (sanity de preservação).
"""
