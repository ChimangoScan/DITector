...
VIZ_SPECS = {
    "openvas": _spec("openvas",
        ...
        cards=[
            {"label":"findings (NVTs)", "expr":"count", "sub":"3 alvos combinados"},
            {"label":"CVSS máximo", "expr":"max:severity", "accent":"#b91c1c",
             "sub_expr":"avg:severity", "sub_prefix":"média "},
            {"label":"com CVE", "expr":"countNonEmpty:cves",
             "sub_expr":"pctNonEmpty:cves", "sub_suffix":" dos findings"},
            {"label":"famílias / portas", "expr":"unique_pair:family,port",
             "sub":"cobertura de superfície"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"severidade (Threat)", "color":"severity",
             "field":"threat"},
            {"kind":"stacked", "title":"severidade × alvo", "color":"severity",
             "field":"threat", "span":2},
            {"kind":"hbar", "title":"top famílias NVT", "field":"family", "limit":8},
            {"kind":"hbar", "title":"portas alvo", "field":"port", "limit":8},
            {"kind":"hbar", "title":"top NVTs (mais disparos)", "field":"name", "limit":8},
        ],
        table=[
            {"title":"NVT", "field":"name", "kind":"wrap"},
            {"title":"Threat", "field":"threat", "kind":"badge-sev"},
            {"title":"CVSS", "field":"severity", "kind":"num1"},
            {"title":"Porta", "field":"port", "kind":"mono"},
            {"title":"Família", "field":"family", "kind":"pkg"},
    ...
    "osv": _spec("osv",
        sev_field=None,
        cards=[
            {"label":"vulns OSV", "expr":"count", "sub":"3 alvos somados"},
            {"label":"CVEs únicas", "expr":"unique:cve",
             "sub_expr":"pct_unique:cve", "sub_suffix":" sem dup"},
            {"label":"pacotes vulneráveis", "expr":"unique_pair:ecosystem,pkg"},
            {"label":"CVSS ≥ 7 (high+)", "expr":"countGte:cvss:7", "accent":"#b91c1c",
             "sub_expr":"pctGte:cvss:7"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"ecossistemas", "color":"eco",
             "field":"ecosystem", "limit":8},
            {"kind":"cvss-bins", "title":"distribuição CVSS", "field":"cvss"},
            {"kind":"stacked", "title":"ecossistema × alvo", "color":"eco",
             "field":"ecosystem"},
            {"kind":"hbar", "title":"top 12 pacotes vulneráveis",
             "field":"pkg_eco", "limit":12, "span":3},
        ],
        table=[
            {"title":"CVE / ID", "field":"cve", "kind":"cve"},
            {"title":"CVSS", "field":"cvss", "kind":"num1"},
            {"title":"Ecossistema", "field":"ecosystem", "kind":"tag-eco"},
            {"title":"Pacote", "field":"pkg", "kind":"pkg"},
            {"title":"Versão", "field":"version", "kind":"mono"},
            {"title":"Alvo", "field":"target", "kind":"plain"},
            {"title":"Resumo", "field":"summary", "kind":"wrap"},
        ],
    ...
    "detect-secrets": _spec("detect-secrets",
        sev_field=None,
        cards=[
            {"label":"leaks detectados", "expr":"count", "sub":"baseline-style, sem verificação"},
            {"label":"verificados", "expr":"countTrue:is_verified", "accent":"#b91c1c"},
            {"label":"tipos únicos", "expr":"unique:type"},
            {"label":"arquivos únicos", "expr":"unique:file"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"tipo (plugin)", "color":"hash",
             "field":"type", "limit":8},
            {"kind":"hbar", "title":"por alvo", "field":"target", "color":"target"},
            {"kind":"hbar", "title":"top 12 arquivos", "field":"file", "limit":12, "span":1},
        ],
        table=[
            {"title":"Tipo", "field":"type", "kind":"badge"},
            {"title":"Alvo", "field":"target", "kind":"plain"},
            {"title":"Arquivo", "field":"file", "kind":"pkg"},
            {"title":"Linha", "field":"line_number", "kind":"num"},
            {"title":"Verif.", "field":"is_verified", "kind":"bool"},
        ],
    ...
    "checkov": _spec("checkov",
        ...
        cards=[
            {"label":"checks falhando", "expr":"count"},
            {"label":"frameworks", "expr":"unique:framework"},
            {"label":"checks únicos", "expr":"unique:check_id"},
            {"label":"arquivos afetados", "expr":"unique:file_path"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"severidade", "color":"severity",
             "field":"severity"},
            {"kind":"hbar", "title":"por framework", "field":"framework"},
            {"kind":"stacked", "title":"severidade × alvo", "color":"severity",
             "field":"severity"},
            {"kind":"hbar", "title":"top 12 checks (mais falhas)", "field":"check_id",
             "limit":12, "span":3},
        ],
        table=[
            ...
            {"title":"Recurso", "field":"resource", "kind":"pkg"},
            {"title":"Arquivo", "field":"file_path", "kind":"pkg"},
            {"title":"Nome", "field":"check_name", "kind":"wrap"},
            {"title":"Alvo", "field":"target", "kind":"plain"},
        ],
        sort_by="severity",
    ),
    "arachni": _spec("arachni",
        ...
        cards=[
            {"label":"issues DAST", "expr":"count"},
            {"label":"CWEs únicas", "expr":"uniqueNonEmpty:cwe"},
            {"label":"URLs únicas", "expr":"unique:url"},
            {"label":"métodos HTTP", "expr":"unique:method"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"severidade", "color":"severity",
             "field":"severity"},
            {"kind":"hbar", "title":"por CWE", "field":"cwe", "span":2},
        ],
        table=[
            {"title":"Issue", "field":"name", "kind":"wrap-bold"},
            {"title":"Sev", "field":"severity", "kind":"badge-sev"},
            {"title":"CWE", "field":"cwe", "kind":"cwe"},
            {"title":"Método", "field":"method", "kind":"mono"},
            {"title":"URL", "field":"url", "kind":"pkg"},
            {"title":"Descrição", "field":"description", "kind":"wrap-muted"},
        ],
        sort_by="severity",
    ),
    "yara": _spec("yara",
        ...
        cards=[
            ...
            {"label":"regras disparadas", "expr":"unique:rule"},
            {"label":"arquivos únicos", "expr":"unique:path"},
            {"label":"alvos com hits", "expr":"unique:target"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"por regra", "color":"hash", "field":"rule"},
            {"kind":"stacked", "title":"regra × alvo", "color":"hash", "field":"rule",
             "span":2},
        ],
        table=[
            {"title":"Regra", "field":"rule", "kind":"cve"},
            {"title":"Alvo", "field":"target", "kind":"plain"},
            {"title":"Arquivo", "field":"path", "kind":"pkg"},
        ],
        sort_by="rule",
    ),
    "retire": _spec("retire",
        ...
        cards=[
            {"label":"vulns JS", "expr":"count"},
            {"label":"componentes únicos", "expr":"unique:component"},
            {"label":"arquivos JS", "expr":"unique:file"},
            {"label":"CVEs distintas", "expr":"uniqueNonEmpty:cve"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"severidade", "color":"severity",
             "field":"severity"},
            {"kind":"hbar", "title":"top libs vulneráveis", "field":"comp_ver", "limit":12},
            {"kind":"stacked", "title":"severidade × alvo", "color":"severity",
             "field":"severity"},
        ],
        table=[
    ...
    "testssl": _spec("testssl",
        ...
        cards=[
            {"label":"findings TLS", "expr":"count"},
            {"label":"severidades distintas", "expr":"unique:severity"},
            {"label":"endpoints", "expr":"unique_pair:ip,port"},
        ],
        callout="Alvos do bench rodam HTTP puro. testssl detecta 'no TLS' como WARN — finding válido mas indica que o cenário do bench não exercita o scanner. Em alvos HTTPS reais esperamos dezenas de findings (ciphers fracos, HSTS ausente, etc.).",
        charts=[
            {"kind":"donut+hbar", "title":"severidade", "color":"severity",
    ...
    "hadolint": _spec("hadolint",
        ...
        cards=[
            {"label":"avisos", "expr":"count"},
            {"label":"códigos únicos", "expr":"unique:code"},
            {"label":"níveis distintos", "expr":"unique:level"},
        ],
        empty_msg="Hadolint não retornou findings. O Dockerfile é reconstruído via <code>docker history --no-trunc</code> e perde COPY/ADD targets — análise limitada.",
        charts=[
            {"kind":"donut+hbar", "title":"nível", "color":"severity", "field":"level"},
            {"kind":"hbar", "title":"códigos disparados", "field":"code", "span":2},
        ],
        table=[
            {"title":"Código", "field":"code", "kind":"cve"},
            {"title":"Nível", "field":"level", "kind":"badge-sev"},
            {"title":"Linha", "field":"line", "kind":"num"},
            {"title":"Mensagem", "field":"message", "kind":"wrap"},
            {"title":"Alvo", "field":"target", "kind":"plain"},
        ],
    ),
    "httpx": _spec("httpx",
        sev_field=None,
        cards=[
            {"label":"endpoints respondidos", "expr":"count"},
            {"label":"servidores únicos", "expr":"uniqueNonEmpty:server"},
            {"label":"TLS detectado", "expr":"countEq:tls_grab:yes"},
        ],
        cards_per_finding=[
            "URL: {url}", "Status: {status_code} · TLS: {tls_grab} · scheme: {scheme}",
            "Servidor: {server}", "Title: {title}", "Tech: {tech}",
            "Content-Type: {content_type}",
        ],
        callout="httpx só rodou contra juice-shop neste bench — alvo node2 não tinha juice-shop levantado quando os dynamics phase-3 dispararam.",
        charts=[],
        table=[
            {"title":"Alvo", "field":"target", "kind":"plain"},
            {"title":"URL", "field":"url", "kind":"pkg"},
            {"title":"Status", "field":"status_code", "kind":"num"},
            {"title":"Servidor", "field":"server", "kind":"mono"},
    ...
    "whispers": _spec("whispers",
        ...
        cards=[
            {"label":"findings", "expr":"count", "sub":"parser de configs estruturados"},
            {"label":"regras únicas", "expr":"unique:rule_id"},
            {"label":"arquivos únicos", "expr":"unique:file"},
            {"label":"chaves únicas", "expr":"unique:key"},
        ],
        charts=[
            {"kind":"donut+hbar", "title":"severidade", "color":"severity",
             "field":"severity"},
            {"kind":"hbar", "title":"top 10 regras", "field":"rule_id", "limit":10},
            {"kind":"stacked", "title":"severidade × alvo", "color":"severity",
             "field":"severity"},
        ],
        table=[
            {"title":"Regra", "field":"rule_id", "kind":"cve"},
            {"title":"Sev", "field":"severity", "kind":"badge-sev"},
            {"title":"Chave", "field":"key", "kind":"pkg"},
            {"title":"Valor", "field":"value", "kind":"mono-muted"},
            {"title":"Arquivo", "field":"file", "kind":"pkg"},
            {"title":"Linha", "field":"line", "kind":"num"},
            {"title":"Alvo", "field":"target", "kind":"plain"},
        ],
...
function _renderHbarItems(items) {
  if (!items.length) return '<p style="color:var(--muted-2);font-size:13px">sem dados</p>';
...
function _cellRender(col, row, spec) {
  ...
  switch (col.kind) {
    ...
    case 'bool': return `<td>${v ? '<b style="color:var(--err)">sim</b>' : 'não'}</td>`;
...
function _renderTable(spec, f) {
  ...
  let html = `<h4 style="font-size:14px;margin:24px 0 10px;color:var(--muted);text-transform:uppercase;letter-spacing:0.04em">Tabela completa</h4>
    <div class="tbl-wrap"><table class="tbl"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div>`;
  if (limit > 0 && rows.length > limit) {
    html += `<div class="tbl-pager">mostrando ${fmtInt(limit)} de ${fmtInt(rows.length)}${sortBy[0]?' (ordenado por '+sortBy[0]+')':''}</div>`;
  }
  return html;
}
...
function _renderBroken(name) {
  const wrapperHints = {
    'jaeles':           'Jaeles esperado: <code>juice-shop-jaeles.json</code> com chaves <code>vuln_url</code>. Saída atual: vazio.',
    'cdxgen':           'cdxgen esperado: <code>juice-shop-cdxgen.cdx.json</code> (CycloneDX SBOM). Saída atual: diretório vazio.',
    'clair':            'Clair esperado: <code>juice-shop-clair.json</code>. Saída atual: vazio (DB CVE não baixou).',
    'dependency-check': 'OWASP DepCheck esperado: <code>dependency-check-report.json/html/sarif</code>. Saída atual: vazio.',
    'govulncheck':      'govulncheck só roda contra binários Go com BuildInfo (Go 1.18+). Saída vazia: nenhum binário Go válido (alvos: Node, PHP, Java).',
    'guarddog':         'GuardDog esperado: <code>juice-shop-guarddog.jsonl</code>. Arquivo vazio: sem typosquatting/install scripts.',
    'kube-linter':      'kube-linter esperado: <code>juice-shop-kube-linter.json</code>. Sem YAML K8s nos alvos.',
    'pip-audit':        'pip-audit esperado: <code>juice-shop-pip-audit.json</code>. Nenhum requirements.txt nos alvos.',
    'secretscanner':    'Deepfence SecretScanner falha com <code>Illegal instruction</code> em CPUs sem AVX2.',
  };
  return `<div class="cards">
    <div class="card" style="grid-column:span 3;text-align:left;background:#fffbeb;border-color:#fde68a">
      <div class="label" style="color:#92400e">Wrapper Docker exitou 0 — sem artefatos no diretório</div>
      <div style="font-size:13px;line-height:1.6;margin-top:10px;color:#78350f">
        ${wrapperHints[name] || ''}<br><br>
        <strong>Status registrado:</strong> <code>ok</code> em metrics.json (wrapper Python não diferencia exit-0-com-output-vazio do exit-0-com-output-presente).
      </div>
    </div>
  </div>`;
}
...
function renderScanner(name) {
  ...
  if (!f.length) {
    const msg = spec.empty_msg || 'Scanner executou mas <strong>não retornou achados</strong> nos 3 alvos do bench.';
