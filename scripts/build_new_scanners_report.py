...
<html lang="pt-BR"><head><meta charset="utf-8"><title>Scanner Comparison — phase 2 + phase 3</title>
<style>{CSS}</style></head><body>
<h1>Phase 2 + 3 scanner comparison</h1>
<p class="lead">Complemento ao <a href="scanner-comparison.html">scanner-comparison.html</a>.
Cobre os <strong>{n_scanners} scanners</strong> adicionados nas fases 2 (12 scanners) e 3 (6 alternativas
sugeridas no painel "Alternativas") executados contra os 3 targets canônicos
(juice-shop, dvwa, webgoat). Total de runs: {len(data)} (ok={ok_count}, falha={fail_count}).
Total de findings: <strong>{total_findings}</strong>. Gerado em {ts}.</p>

<div class="legend">
  <span><span class="badge phase2">P2</span> phase 2 (12 scanners — sinal único vs original)</span>
  <span><span class="badge phase3">P3</span> phase 3 (6 alternativas suggeridas no painel)</span>
  <span><span class="badge ok">ok</span> sucesso</span>
  <span><span class="badge err">erro</span> falha</span>
</div>

<div class="card">
  <h2>Matriz de findings</h2>
  {render_summary_table(data)}
</div>

<div class="card">
  <h2>Métricas detalhadas (wall time, CPU, RAM, disco)</h2>
